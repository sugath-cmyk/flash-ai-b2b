...
export class WidgetChatService {
  ...
  private async getStoreContext(storeId: string, productContext?: any): Promise<string> {
    ...
    if (discountsResult.rows.length > 0 || offersResult.rows.length > 0) {
      ...
      context += `(Mention these ONLY when customer asks about discounts, deals, or promotions)\n\n`;

      // Shopify discounts
      discountsResult.rows.forEach((discount, idx) => {
        context += `${idx + 1}. CODE: ${discount.code}\n`;
        context += `   ${discount.title}\n`;
        if (discount.description) {
          context += `   Details: ${discount.description}\n`;
        }

        // Value
        if (discount.value_type === 'percentage') {
          context += `   Save: ${discount.value}% off\n`;
        } else if (discount.value_type === 'fixed_amount') {
          context += `   Save: ₹${discount.value} off\n`;
        } else if (discount.value_type === 'free_shipping') {
          context += `   Benefit: Free shipping\n`;
        }

        // Minimum requirements
        if (discount.minimum_requirements?.minimum_purchase_amount) {
          context += `   Min purchase: ₹${discount.minimum_requirements.minimum_purchase_amount}\n`;
        }

        // Product/collection restrictions
        if (discount.entitled_product_ids && discount.entitled_product_ids.length > 0) {
          context += `   Applies to: Specific products only\n`;
        } else if (discount.entitled_collection_ids && discount.entitled_collection_ids.length > 0) {
          context += `   Applies to: Specific collections only\n`;
        } else {
          context += `   Applies to: All products\n`;
        }

        // Expiry
        if (discount.ends_at) {
          const endDate = new Date(discount.ends_at);
          context += `   Valid until: ${endDate.toLocaleDateString('en-IN', {
            day: 'numeric',
            month: 'short',
            year: 'numeric',
          })}\n`;
        } else {
          context += `   Valid: No expiry\n`;
        }
      ...
      // Manual offers
      const shopifyCount = discountsResult.rows.length;
      offersResult.rows.forEach((offer, idx) => {
        context += `${shopifyCount + idx + 1}. ${offer.title}\n`;
        context += `   ${offer.description}\n`;

        if (offer.code) {
          context += `   CODE: ${offer.code}\n`;
        }

        // Value
        if (offer.discount_type === 'percentage' && offer.discount_value) {
          context += `   Save: ${offer.discount_value}% off\n`;
        ...
        if (offer.minimum_purchase) {
          context += `   Min purchase: ₹${offer.minimum_purchase}\n`;
        }
        ...
        if (offer.terms_and_conditions) {
          context += `   Terms: ${offer.terms_and_conditions}\n`;
        }

        context += `\n`;
      });

      context += `IMPORTANT: Only mention these discounts when customer specifically asks about:\n`;
      context += `- "Do you have any discounts?"\n`;
      context += `- "Are there any offers?"\n`;
      context += `- "Do you have a coupon code?"\n`;
      context += `- "Any deals available?"\n`;
      context += `\n`;
    }
...
CRITICAL: Be PASSIVE about discounts and offers.

✅ ONLY mention discounts when customer explicitly asks:
...
❌ DO NOT proactively suggest discounts during:
...
When customer asks about discounts:
✅ List all active offers with codes
✅ Explain how to apply codes at checkout
✅ Mention minimum purchase requirements
✅ Clarify expiry dates
✅ Be specific about what products/collections they apply to

Example (when asked):
"Yes! We have some great offers right now:

1. WINTER25 - Get 25% off on all products
   Min purchase: ₹1,000
   Valid until: Jan 31, 2026

2. FREESHIP - Free shipping on orders ₹500+
   Valid for all products

Just enter the code at checkout to apply the discount! 🎉"
